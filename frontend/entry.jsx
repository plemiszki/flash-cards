...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (messageNode) {
    createRoot(messageNode).render(<Message />);
  }
  ...
  if (quizDetailsNode) {
    createRoot(quizDetailsNode).render(<QuizDetails />);
  }

  const spanishNounNode = document.getElementById("spanish-noun-details");
  if (spanishNounNode) {
    createRoot(spanishNounNode).render(<SpanishNounDetails />);
  }

  const spanishVerbNode = document.getElementById("spanish-verb-details");
  if (spanishVerbNode) {
    createRoot(spanishVerbNode).render(<SpanishVerbDetails />);
  }

  const spanishAdjectiveNode = document.getElementById(
    "spanish-adjective-details"
  );
  if (spanishAdjectiveNode) {
    createRoot(spanishAdjectiveNode).render(<SpanishAdjectiveDetails />);
  }

  const spanishMiscNode = document.getElementById("spanish-misc-details");
  if (spanishMiscNode) {
    createRoot(spanishMiscNode).render(<SpanishMiscDetails />);
  }

  const frenchNounNode = document.getElementById("french-noun-details");
  if (frenchNounNode) {
    createRoot(frenchNounNode).render(<FrenchNounDetails />);
  }

  const frenchVerbNode = document.getElementById("french-verb-details");
  if (frenchVerbNode) {
    createRoot(frenchVerbNode).render(<FrenchVerbDetails />);
  }

  const frenchAdjectiveNode = document.getElementById(
    "french-adjective-details"
  );
  if (frenchAdjectiveNode) {
    createRoot(frenchAdjectiveNode).render(<FrenchAdjectiveDetails />);
  }

  const frenchCountryNode = document.getElementById("french-country-details");
  if (frenchCountryNode) {
    createRoot(frenchCountryNode).render(<FrenchCountryDetails />);
  }

  const frenchCityNode = document.getElementById("french-city-details");
  if (frenchCityNode) {
    createRoot(frenchCityNode).render(<FrenchCityDetails />);
  }

  const frenchMiscNode = document.getElementById("french-misc-details");
  if (frenchMiscNode) {
    createRoot(frenchMiscNode).render(<FrenchMiscDetails />);
  }

  const quizRunNode = document.getElementById("quiz-run");
  if (quizRunNode) {
    createRoot(quizRunNode).render(<QuizRun />);
  }
      ...
      searchCriteria: {
        fields: [
          ...
          { name: "answer", fuzzy: true, columnWidth: 10 },
          {
            ...
            responseArrayName: "tags",
            dbName: "card_tags.tag_id",
            columnWidth: 8,
          },
        ],
      },
