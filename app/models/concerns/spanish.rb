    ...
    if quiz_question.tag_id
      ...
      until !tagged_words.empty? do
        ...
        if tagged_words.empty?
          new_tagged_words = SpanishMisc.includes(:tags).where(tags: { id: Tag.find(quiz_question.tag_id) })
          raise "No Spanish Misc Words with Tag: #{Tag.find(quiz_question.tag_id).name}" if new_tagged_words.empty?
